...
//! integration.
//!
//! For **getting started** with using `rust-bio`, see [the `Getting started` section
//! below](#getting-started). For navigating the documentation of the available modules, see [the
//! `Modules` section below](#modules). If you want to contribute to `rust-bio`, see [the `Contribute` section in the repo](https://github.com/rust-bio/rust-bio#contribute).
//!
...
//! cd hello_world
//! ```
//! in your terminal. The flag `--bin` tells Cargo to create an executable project instead of a
//! library. In [this section](https://doc.rust-lang.org/nightly/book/hello-cargo.html#a-new-project) of the Rust docs, you find details about what Cargo just created for you.
//!
...
//! ```
//!
//! Documentation and further examples for each module can be found in the module descriptions
//! below.
//!
//!
...
//! # Benchmarks
//!
//! Since Rust-Bio is based on a compiled language, similar performance to C/C++ based libraries can
//! be expected. Indeed, we find the pattern matching algorithms of Rust-Bio to perform in the range
//! of the C++ library Seqan:
//!
...
//!
//! We measured 10000 iterations of searching pattern `GCGCGTACACACCGCCCG` in the sequence of the
//! hg38 MT chromosome. Initialization time of each algorithm for the given pattern was included in
//! each iteration. Benchmarks were conducted with *Cargo bench* for Rust-Bio and *Python timeit*
//! for Seqan on an Intel Core i5-3427U CPU. Benchmarking Seqan from *Python timeit* entails an
//! overhead of 1.46ms for calling a C++ binary. This overhead was subtracted from above Seqan run
//! times. Note that this benchmark only compares the two libraries to exemplify that Rust-Bio has comparable speed to C++ libraries: all used algorithms have their advantages for specific text and pattern structures and lengths (see [the pattern matching section in the documentation](https://docs.rs/bio/0.28.2/bio/pattern_matching/index.html))./!
